        ...
        initializePolling(connection, (error, data) => {
          ...
          startPolling(connection, (error, data) => {
            ...
            stopPolling(connection, (error, data) => {
              ...
              if (!thermostatData.deviceId) {
                callback(new Error('Received no data from device!'));
                return;
              }
              writeDataToKinesisStream(STREAM_NAME, thermostatData, (error, data) => {
                if (error) {
                  callback(error);
                  return;
                }
...
function writeDataToKinesisStream(streamName, data, callback) {
  const streamData = {
    StreamName: streamName,
    PartitionKey: uuidv1(),
    Data: JSON.stringify(data)
  }
  kinesis.putRecord(streamData, callback);
}
